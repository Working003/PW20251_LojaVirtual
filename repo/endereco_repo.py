...
def atualizar_endereco(endereco: Endereco) -> bool:
    ...
    cursor.execute(UPDATE_ENDERECO, 
        (endereco.cep, endereco.logradouro, endereco.numero, endereco.complemento, endereco.bairro, endereco.cidade, endereco.estado, endereco.id))
    conexao.commit()
    conexao.close()
...
def obter_enderecos_por_pagina(limite: int, offset: int) -> list[Endereco]:
    """Obtém uma lista de enderecos com paginação."""
    conexao = obter_conexao()
    cursor = conexao.cursor()
